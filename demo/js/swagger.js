...
box.querySelector('pre').textContent='';
try{const data=await LibreEchoDemo.request(url,{method:op.method,headers,body});
box.querySelector('.response-status').textContent='200 OK · browser demo';
box.querySelector('.response-status').className='response-status success';
box.querySelector('pre').textContent=JSON.stringify({ok:true,data,error:null},null,2)}catch(e){box.querySelector('.response-status').textContent='Request failed';
...
LibreEchoDemo.request('/config').catch(()=>{});
